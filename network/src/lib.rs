...
pub async fn with_config(
    config: Config,
) -> Result<(Client, mpsc::Receiver<Event>), Box<dyn std::error::Error>> {
    let (command_sender, command_receiver) = mpsc_tuple(&config);

    let (event_sender, event_receiver) = mpsc_tuple(&config);

    let eventloop = eventloop::EventLoop::new(event_sender, command_receiver).await?;
...
pub async fn new() -> Result<(Client, mpsc::Receiver<Event>), Box<dyn std::error::Error>> {
